...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ name: string; value: number; color: string }>; label?: string }) {
  ...
  return (
    <div className="bg-[#1a2235] border border-[#2d4060] rounded p-2.5 text-[11px] font-mono shadow-xl">
      <div className="text-[#f5f6f7] mb-1.5">{label}</div>
      {payload.map(p => (
        <div key={p.name} className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-sm inline-block" style={{ background: p.color }} />
          <span className="text-[#f5f6f7]">{p.name}:</span>
      ...
      <div className="bg-[#111827] border border-[#1e2d45] rounded-lg p-4">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data} margin={{ top: 10, right: 16, left: 0, bottom: 60 }}>
            ...
            <XAxis
              dataKey="metric"
              tick={{ fill: '#f5f6f7', fontSize: 9, fontFamily: 'Space Mono' }}
              angle={-35}
              textAnchor="end"
              interval={0}
              tickLine={false}
            ...
            <Bar dataKey="base" name={base.displayName} fill="#3b82f6" radius={[2, 2, 0, 0]}>
              ...
              <LabelList dataKey="baseLabel" position="top" style={{ fill: '#f5f6f7', fontSize: 8, fontFamily: 'Space Mono' }} />
            </Bar>
        ...
        <p className="text-[9px] text-[#f5f6f7] font-mono mt-2 text-center">
